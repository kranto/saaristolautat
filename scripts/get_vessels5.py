...
location_uri = "https://meri.digitraffic.fi/api/v1/locations/latitude/%s/longitude/%s/radius/%s/from/%s" % (centerLat, centerLng, radius, dateStr)

vessel_json = urllib2.urlopen("https://meri.digitraffic.fi/api/v1/metadata/vessels").read()
location_json = urllib2.urlopen(location_uri).read()

vessel_data = json.loads(vessel_json)
passangerVessels = [a for a in vessel_data if a[u'shipType'] == 60 or a[u'shipType'] == 0 ]
vesselDict = {}
...
connection = sqlite3.connect("lauttalive.db")
c = connection.cursor()
...
c.execute('select * from snapshots where timestamp > ? order by mmsi, timestamp desc;', (timelimit,))
